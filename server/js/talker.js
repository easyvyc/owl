					...
					if(visobj.length > 0){
						...
						$(this.visitors).prepend("" +
							"<div class='visitor radius3' rel='"+visit.id+"' id='visit_"+this.id+"_"+visit.id+"'>" +
								...
								"<div class=\"from\">" + "<img src=\"images/countries/"+visit.country_code+".gif\" class=\"vam\" alt=\"\" />" + visit.city + "</div>" +
								"<div class=\"ip\">" + visit.ipaddress + "</div>" +
								"<div class=\"referer\">" + visit.referer + "</div>" + 
								"<div class=\"pages\">" + visit.url + "(" + visit.page_count + ")" + "</div>" + 
								"<div class=\"time\">" + visit.visit_time + "</div>" +
								"<div class=\"write\"><a href='javascript: void(chat(\""+this.id+"\", \""+visit.id+"\"))'>Write</a></div>" +
							"</div>");
						$("#messages").append("<form class='chat hide' id='chat_"+this.id+"_"+visit.id+"' action='javascript: void(sendMessage(\""+this.id+"\", \""+visit.id+"\"));'><div id='msgs_"+this.id+"_"+visit.id+"'></div><input type='text' class='txt vam radius3' name='txt' /> <input type='submit' value='send' class='btn vam radius3' /><div class='typeing'></div></form>");
					}
					visobj.addClass('new');					
				}
		}
...
var $server = {
	...
	add:function(conts){
		$("#msgs_"+conts.website_id+"_"+conts.visitor_id).append("<div class='msg radius3 dir" + conts.direction + "'><em>" + conts.create_date + "</em><p>" + conts.message + "</p></div>");
